...
def example(save_path):
    ...
    for n in range(n_trials):
        ...
        for i in np.arange(0, N_iterations):
            ...
            Optimal_response[n, i], error = position_uncertainty(X, lambda_coef=lambda_coef, target_width=target_width)
    ...
    ax = fig.add_subplot(1,1,1) #stops per trial
    for i in range(n_trials):
        plt.plot(Optimal_response[i, :], gt_positions, "r", alpha=0.1)

    plt.plot(avg_optimal_response, gt_positions, "g")
    plt.plot(np.arange(0,400), np.arange(0,400), "k--", label="Unity")
    plt.xlabel("Target", fontsize=20)
