...
def data():
    ...
    coef3s = np.linspace(-0.01, -0.05, 10)
    coef4s = np.linspace(1, 10, 12)

    parameters_powers_conditions = np.zeros((n_subjects, len(coef3s), len(coef4s)))
    parameters_powers_track_length = np.zeros((n_subjects, len(coef3s), len(coef4s)))
    parameters_powers_interaction = np.zeros((n_subjects, len(coef3s), len(coef4s)))
    ...
    for counter3, coef3 in enumerate(coef3s):
        for counter4, coef4 in enumerate(coef4s):
            ...
            for n in range(0, n_subjects):
                ...
                parameters_powers_conditions[n-1, counter3, counter4] = power_condition
                parameters_powers_track_length[n-1, counter3, counter4] = power_track_length
                parameters_powers_interaction[n-1, counter3, counter4] = power_interaction
